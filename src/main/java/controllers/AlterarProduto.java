...
public class AlterarProduto extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("alterarProduto.jsp");

		String id = req.getParameter("id");
		setIdProduto(id);
		if (id != null && id != "") {
			List<Produto> produtoBuscaLista = DAOproduto.buscar(id);

			if (produtoBuscaLista.size() > 0) {

				System.out.println("Produto encontrado. Dados abaixo:");
				System.out.println(produtoBuscaLista.get(0).toString());
				Produto produtoBusca = produtoBuscaLista.get(0);

				req.setAttribute("produtoBusca", produtoBusca);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (listaErros.isEmpty()) {
			...
			produtoDadosAlterados = new Produto(nome, undVenda, precoCDoub, precoVDoub, margemLDoub, quantDoub);

			try {
				DAOproduto.alterar(produtoDadosAlterados);
				...
				req.setAttribute("SucessMsg", "Alteração no cadastro realizada com sucesso!");
			...
			System.out.println("Não foi possivel realizar a alteração no cadastro. Infos no log");
